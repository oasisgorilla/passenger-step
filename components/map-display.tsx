    ...
    const initializeMap = () => {
      try {
        ...
        const drawRelatedGroupPolylines = (groupId: string) => {
          ...
          const groupDestinations = group.destinationIds
            .map((id) => destMap[id])
            .filter(Boolean);
          ...
          marker.addListener("mouseover", () => {
            ...
            if (dest.relatedGroupIds && dest.relatedGroupIds.length > 0) {
              ...
              // For now, we'll show the first one that's not already active
                ...
                const groupToShow =
                  dest.relatedGroupIds.find(
                    (id) => id !== activeGroupRef.current
                  ) || dest.relatedGroupIds[0];
